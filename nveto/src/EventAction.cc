...
void EventAction::EndOfEventAction(const G4Event* aEvent)
{
    ...
    for(int i=0; i<vNumberOfHit; i++) {
		...
		Edep = aHit->GetEnergy()/CLHEP::MeV;
		if (Edep > 0.) {
			analysisManager->FillNtupleIColumn(0,0,aHit->GetType());
